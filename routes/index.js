...
module.exports = function (db) {
	...
	router.get('/', function(req, res, next){
		res.redirect("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxdeb5dc277a2c46bf&redirect_uri=http://wx.oppo.com/oppootherfirm10/login/&response_type=code&scope=snsapi_base#wechat_redirect");
	})
...
function getNowFormatDate() {
	...
	router.get('/wx', function(req, res, next) {
        ...
        if(req.query.share) {
        	...
        	if(req.query.code != ""){
        		var string1 = 'jsapi_ticket='+config.jsticket+'&noncestr=' + random_str +'&timestamp=' + timestamp + '&url=http://oppo10.nplusgroup.net/login?code=' + code + "&state=";
        	}
        	else {
        		var string1 = 'jsapi_ticket='+config.jsticket+'&noncestr=' + random_str +'&timestamp=' + timestamp + '&url=http://oppo10.nplusgroup.net/login?from=' + req.query.from;
