...
function wx_init(data) {
  ...
  wx.ready(function () {
    wx.onMenuShareTimeline({
      ...
      link: 'http://wx.oppo.com/oppootherfirm10/login',
...
function wx_process() {
  wx.ready(function () {
    wx.onMenuShareTimeline({
      ...
      link: 'http://oppo10.nplusgroup.net/my_museum?name=' + encodeURI(user_name) + '&museum=' + panorama + '&time=' + user_time + '&id=' + user_id + '&code=' + $("#wx_code").val(), // 分享链接
      imgUrl: "http://oppo10.nplusgroup.net/image/500px.jpg", // 分享图标
      success: function () {
        // 用户确认分享后执行的回调函数
      },
